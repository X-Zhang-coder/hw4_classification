...
class KMeansClusterDigit(BaseFeatureExtractor):
    ...
    def fit(self, trainingData):
        ...
        for _ in range(self.num_iterations):
            for tag in range(self.num_cluster):
                self.clusters[tag] = np.mean(trainingData[cluster_no == tag], axis=0)
            for i in range(n):
                cluster_no[i] = np.argmin(np.linalg.norm(self.clusters - trainingData[i], axis=1))
